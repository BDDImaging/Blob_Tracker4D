...
public class ABSnakeFast {
   ...
   RandomAccessibleInterval<FloatType> originalImage;
    ...
    Point2d compute_displ1D(int num, double Edge_Threshold, double dist_plus, double dist_minus, int dir) {
        ...
        CannyDeriche1D canny = new CannyDeriche1D(image_line, config.getAlpha());
...
//        crm = crm * coeff_crossing;
    ...
    private ImageProcessor grad2d_deriche(ImageProcessor iDep, double alphaD) {
        ...
        for (i = 0; i < height; ++i) {
            ...
            a2[iwidth + 1] = an1 * a1[iwidth + 1]
                    + an2 * a1[iwidth] - ad1 * a2[iwidth];
            for (j = 2; j < width; ++j) {
                a2[iwidth + j] = an1 * a1[iwidth + j] + an2 * a1[icol_1 + j]
        ...
        for (i = 0; i < height; ++i) {
            ...
            for (j = col_3; j >= 0; --j) {
                a3[iwidth + j] = an3 * a1[icol_1 + j] + an4 * a1[icol_2 + j]
